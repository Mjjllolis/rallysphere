...
export const createStripeConnectAccount = functions.https.onCall(
  ...
  async (data: any, context: any) => {
    ...
    console.log("CreateStripeConnectAccount called with context:", {
      hasAuth: !!context.auth,
      uid: context.auth?.uid,
      token: context.auth?.token ? "present" : "missing",
      rawAuth: context.rawRequest?.headers?.authorization ? "present" : "missing"
    });
  ...
  try {
    ...
    const accountLink = await stripe.accountLinks.create({
      account: account.id,
      refresh_url: `rallysphere://stripe-connect/refresh?clubId=${clubId}`,
      return_url: `rallysphere://stripe-connect/return?clubId=${clubId}`,
